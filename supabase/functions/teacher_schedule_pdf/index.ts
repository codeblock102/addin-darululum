...
serve(async (req) => {
  ...
  try {
    ...
    if (scheduleError) {
      return new Response(
        JSON.stringify({ error: 'Failed to fetch schedule', details: scheduleError }),
        {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        }
    ...
    // Add title
    doc.setFontSize(18)
    doc.text(`Teacher Schedule: ${teacherData.name}`, 105, 20, { align: 'center' })
    doc.setFontSize(12)
    ...
    dayOrder.forEach(day => {
      if (schedulesByDay[day]) {
        // Add day header
        tableData.push([{ content: day, colSpan: 4, styles: { fontStyle: 'bold', fillColor: [240, 240, 240] } }])
