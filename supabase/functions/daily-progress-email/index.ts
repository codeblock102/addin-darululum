  ...
  try {
    ...
    try {
      ...
      if (body) {
        ...
        if (
          parsedBody.scope === "class" || parsedBody.scope === "school" ||
          parsedBody.scope === "section"
        ) {
    ...
    console.log(
      `Function invoked. Trigger source: ${triggerSource}, Timestamp: ${timestamp}`,
    );
    ...
    const scheduleEnabled =
      (settingsMap.get("email_schedule_enabled") ?? "true") !== "false";
    ...
    try {
      ...
      if (resp.ok) {
        ...
        logoImgHtml =
          `<div style="text-align:center;margin-top:16px;"><img src="data:${contentType};base64,${base64}" alt="Dār Al-Ulūm Montréal" style="max-width:200px;height:auto;"/></div>`;
      } else {
        logoImgHtml =
          `<div style=\"text-align:center;margin-top:16px;\"><img src=\"${logoUrl}\" alt=\"Dār Al-Ulūm Montréal\" style=\"max-width:200px;height:auto;\"/></div>`;
      }
    } catch (_e) {
      logoImgHtml =
        `<div style=\"text-align:center;margin-top:16px;\"><img src=\"${logoUrl}\" alt=\"Dār Al-Ulūm Montréal\" style=\"max-width:200px;height:auto;\"/></div>`;
    ...
    const timeRemainingMs = () =>
      Math.max(0, maxRuntimeMs - (Date.now() - startedAtMs));
    ...
    if (triggerSource !== "scheduled") {
      ...
      const { data: teacherProfile, error: teacherProfileErr } =
      ...
      if (teacherProfileErr || !teacherProfile?.madrassah_id) {
        console.error(
          "Invoking user's profile missing madrassah_id",
          teacherProfileErr,
        );
        return new Response(
          JSON.stringify({ error: "Profile missing madrassah_id" }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          },
      ...
      const caps = Array.isArray((teacherProfile as any).capabilities)
        ? (teacherProfile as any).capabilities as string[]
        : [];
      ...
      if (!isAdminRole && requestScope === "section") {
        ...
        if (!reqSec || !teacherSec || reqSec !== teacherSec) {
          return new Response(
            JSON.stringify({ error: "Forbidden: section mismatch" }),
            {
              status: 403,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
            },
    ...
    try {
      ...
      console.log(
        "Could not log to email_logs table (table may not exist):",
        logError,
      );
    }
    ...
    console.log(
      `Querying progress for local day ${todayLocalDate} (${tzForReport})`,
    );
    ...
    if (requestScope === "section" && requestSection) {
      ...
      allowedSectionStudentIds = Array.from(
        new Set((sectionStudents || []).map((s: { id: string }) => s.id)),
      );
    }
    ...
    if (requestScope === "class" && requestClassId) {
      ...
      const { data: cls } = await supabaseService
        ...
        .maybeSingle();
      const classStudentIds: string[] = Array.from(
        new Set((cls?.current_students || []).filter(Boolean)),
      );
      if (classStudentIds.length > 0) {
        progressQuery = progressQuery.in("student_id", classStudentIds);
      } else {
        // No students => empty result
        progressQuery = progressQuery.in("student_id", [
    ...
    const { data: subsDaily, error: subsErr } = await supabaseService
      ...
      .select(
        "assignment_id, student_id, status, submitted_at, graded_at, grade, feedback, created_at",
      )
    ...
    const progressStudentIds = [
      ...new Set((progressRecords || []).map((r) => r.student_id)),
    ];
    const submissionStudentIds = [
      ...new Set((subsDaily || []).map((s) => s.student_id)),
    ];
    const allStudentIds = [
      ...new Set([...progressStudentIds, ...submissionStudentIds]),
    ];

    let studentsQuery = supabaseService
      .from("students")
      .select(
        "id, name, guardian_contact, guardian_name, guardian_email, madrassah_id",
      )
      .in(
        "id",
    ...
    if (requestScope === "class" && requestClassId) {
      ...
      const allowedIds = Array.from(
        new Set((cls2?.current_students || []).filter(Boolean)),
      );
      if (allowedIds.length > 0) {
        studentsQuery = studentsQuery.in("id", allowedIds);
      } else {
        studentsQuery = studentsQuery.in("id", [
          "00000000-0000-0000-0000-000000000000",
        ]);
      }
    } else if (requestScope === "section" && requestSection) {
    ...
    const studentProgressMap = new Map<
      string,
      { student: Student; progresses: ProgressRecord[] }
    >();
    ...
    for (const record of (progressRecords || []) as ProgressRecord[]) {
      ...
      if (!student) {
        console.warn(
          `Student with ID ${record.student_id} not found for a progress record.`,
        );
        continue;
      }
    ...
    const filteredSubs = (subsDaily || []).filter((s) =>
      studentsMap.has(s.student_id)
    );
    const assignmentIds = [
      ...new Set(filteredSubs.map((s) => s.assignment_id)),
    ];
    ...
    if (assignmentIds.length > 0) {
      ...
      if (assignsError) {
        console.error(
          "Error fetching assignments for academic section:",
          assignsError,
        );
      } else {
        assignmentMap = new Map(
          (assigns || []).map((
            a: { id: string; title: string; due_date: string | null },
          ) => [a.id, { id: a.id, title: a.title, due_date: a.due_date }]),
        );
      }
    }

    const studentAcademicMap = new Map<
      string,
      Array<
        {
    ...
    const allClasses: Array<
      { id: string; name: string; current_students: string[] | null }
    > = (classRows || []) as any[];
    ...
    const classesForSummary = (requestScope === "class" && requestClassId)
      ...
      ? allClasses.filter((c) =>
        (c.current_students || []).some((sid: string | null) =>
          !!sid && allowedSectionStudentIds.includes(sid)
        )
      )
      : allClasses;
    ...
    for (const cls of classesForSummary) {
      ...
      const eligibleIds =
        (requestScope === "section" && allowedSectionStudentIds.length > 0)
          ? classStudentIds.filter((sid) =>
            allowedSectionStudentIds.includes(sid)
          )
          : classStudentIds;
      // Filter to students with activity today or recent submissions
      const activeStudentIds = eligibleIds.filter((sid) =>
        studentsMap.has(sid) &&
        (studentProgressMap.has(sid) || studentAcademicMap.has(sid))
      );
      ...
      for (const sid of activeStudentIds) {
        ...
        for (const p of progresses) {
          ...
          totalPagesByStudent.set(
            sid,
            (totalPagesByStudent.get(sid) || 0) + (p.pages_memorized || 0),
          );
        }
      }
      ...
      if (totalPagesByStudent.size > 0) {
        const topEntry =
          Array.from(totalPagesByStudent.entries()).sort((a, b) =>
            b[1] - a[1]
          )[0];
      ...
      const assignmentAggregate = new Map<
        string,
        { title: string; due: string | null; count: number }
      >();
      for (const sid of activeStudentIds) {
    ...
    for (const { student, progresses } of studentProgressMap.values()) {
      ...
      try {
        ...
        if (parentsError) {
          console.error(
            `Error fetching parents for student ${studentUuid}:`,
        ...
        console.error(
          `Exception in parent email lookup for student ${student.id}:`,
          (e as Error).message,
        );
      }
      ...
      try {
        ...
        const { data: subs, error: subsErr } = await supabaseService
          .from("teacher_assignment_submissions")
          .select(
            "assignment_id, status, grade, feedback, submitted_at, graded_at",
          )
        ...
        if (!subsErr && subs && subs.length > 0) {
          const assignmentIds = Array.from(
            new Set(
              (subs as SubmissionDb[]).map((s) => s.assignment_id).filter(
                Boolean,
              ),
            ),
          );
          ...
          try {
            const { data: directAssigns, error: directErr } =
              await supabaseService
                .from("teacher_assignments")
                .select("id, title, due_date")
    ...
    if (emailPayloads.length > 0) {
      ...
      console.log(
        `Sending ${emailPayloads.length} emails in ${batches.length} batch(es)`,
      );
      ...
      for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        ...
        try {
          ...
          const batchPayload = batch.map((
            { student_name, guardian_email, ...emailData },
          ) => emailData);
          ...
          if (error) {
            ...
            for (let i = 0; i < batch.length; i++) {
              ...
              if (result && result.id && !result.error) {
                ...
                console.log(
                  `Email sent to ${payload.guardian_email} for student ${payload.student_name}`,
                );
              } else {
                // Failed
                emailsSkipped++;
                ...
                console.error(
                  `Failed to send email to ${payload.guardian_email} for student ${payload.student_name}:`,
                  errorMsg,
                );
              }
            }
    ...
    try {
      if (ADMIN_EMAILS.length > 0 && timeRemainingMs() > 15000) {
        await withTimeout(
          ...
          20000,
          "Admin summary email send (post-guardians)",
        );
      ...
      await supabaseService
        ...
        .insert({
          ...
          message: `Sent ${emailsSent} emails, skipped ${emailsSkipped}${
            endedDueToTimeout ? " (timed out and ended early)" : ""
          }`,
        });
    } catch (logError) {
    ...
    console.log(
      `Email sending process completed successfully. Sent: ${emailsSent}, Skipped: ${emailsSkipped}`,
    );
