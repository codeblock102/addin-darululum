 ...
 * @summary Fetches summary statistics for a teacher's dashboard.
 */
import { useQuery } from "@tanstack/react-query";
import { supabase } from "@/integrations/supabase/client.ts";
import { SummaryData } from "@/types/teacher.ts";
    ...
    queryFn: async (): Promise<SummaryData> => {
      ...
      try {
        ...
        return {
          ...
          upcomingRevisions: 0,
          completionRate: progressEntries?.length
            ? Math.round(
              (progressEntries.length / (students?.length || 1)) * 100,
            )
            : 0,
