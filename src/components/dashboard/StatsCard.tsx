...
export const StatsCard = ({ title, value, icon, trend }: StatsCardProps) => {
  ...
  return (
    <Card className={`stats-card ${isAdmin ? 'glass-effect' : ''} overflow-hidden`}>
      <div className="p-6">
        <div className="flex items-center justify-between">
          <h3 className={`text-sm font-medium ${isAdmin ? 'text-amber-400' : 'text-muted-foreground'}`}>
            {title}
          </h3>
          {icon && (
            <div className={`${isAdmin ? 'bg-amber-500/10 p-1.5 rounded-full' : ''}`}>
              <div className={`${isAdmin ? 'text-amber-500' : 'text-muted-foreground'}`}>
                {icon}
              </div>
            </div>
          )}
