...
export const useQuranData = () => {
  ...
  const [surahListForSelectedJuz, setSurahListForSelectedJuz] = useState<
    string | null
  >(null);
  const [isLoadingSurahList, setIsLoadingSurahList] = useState<boolean>(false);
  const [errorFetchingSurahList, setErrorFetchingSurahList] = useState<
    string | null
  >(null);

  const [parsedSurahNumbers, setParsedSurahNumbers] = useState<number[] | null>(
    null,
  ); // Stores numbers from surah_list string

  const [surahDetailsForSelectedJuz, setSurahDetailsForSelectedJuz] = useState<
    SurahData[]
  >([]);
  const [isLoadingSurahDetails, setIsLoadingSurahDetails] = useState<boolean>(
    false,
  );
  const [errorFetchingSurahDetails, setErrorFetchingSurahDetails] = useState<
    string | null
  >(null);
  ...
  const { data: allSurahsData, isLoading: allSurahsLoading } = useQuery<
    SurahData[]
  >({
  ...
  const parseSurahListToNumbers = useCallback(
    (
      listStrings: (string | null)[],
      allSurahs: SurahData[] | undefined,
    ) => {
      if (
        !listStrings || listStrings.length === 0 || !allSurahs ||
        allSurahs.length === 0
      ) {
        setParsedSurahNumbers(null);
        return;
      }
      console.log(`Parsing surah list strings:`, listStrings);
      const numbers: number[] = [];
      listStrings.forEach((listString) => {
        ...
        parts.forEach((part) => {
          ...
          const matchedSurah = allSurahs.find((s) =>
          ...
          if (part.includes("-")) {
            ...
            if (isNaN(startNum)) {
              const startSurahObj = allSurahs.find((s) =>
                s.name.toLowerCase() === startStr.toLowerCase()
              );
              if (startSurahObj) {
                startNum = startSurahObj.surah_number;
              } else {
                console.warn(
                  `Could not resolve start of range: "${startStr}" in part "${part}"`,
                );
                return; // Skip this part
              }
            }
            if (isNaN(endNum)) {
              const endSurahObj = allSurahs.find((s) =>
                s.name.toLowerCase() === endStr.toLowerCase()
              );
              if (endSurahObj) {
                endNum = endSurahObj.surah_number;
              } else {
                console.warn(
                  `Could not resolve end of range: "${endStr}" in part "${part}"`,
                );
                return; // Skip this part
              }
            }
            ...
            if (!isNaN(startNum) && !isNaN(endNum) && startNum <= endNum) {
              ...
              console.warn(
                `Invalid or unresolvable range after attempting to resolve names: "${part}"`,
              );
            }
          } else {
            ...
            console.warn(
              `Could not parse "${part}" as a number, a recognized surah name, or a valid range.`,
            );
          }
        });
      });
  ...
  const fetchSurahDetailsByNumbers = useCallback(
    async (numbersToFetch: number[]) => {
      ...
      try {
        ...
        console.error(
          "Unexpected error fetching surah details by numbers:",
          errorMessage,
        );
  ...
  const fetchAndStoreSurahList = async (juzNumber: number) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error
        ...
        : "An unknown error occurred";
      console.error(
        `Unexpected error fetching surah_list for Juz ${juzNumber}:`,
        errorMessage,
      );
  ...
  useEffect(() => {
    if (surahListForSelectedJuz && allSurahsData && allSurahsData.length > 0) {
      parseSurahListToNumbers(surahListForSelectedJuz.split(", "), allSurahsData);
  ...
  return {
    ...
    isLoadingSurahs: isLoadingSurahDetails || isLoadingSurahList ||
      allSurahsLoading,
