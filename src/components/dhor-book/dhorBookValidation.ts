...
export const DailyActivityFormSchema = z.object({
  ...
  memorization_quality: z.enum(["excellent", "good", "average", "needsWork", "horrible"]).optional(), // For Sabaq quality
  ...
  sabaq_para_memorization_quality: z.enum(["excellent", "good", "average", "needsWork", "horrible"]).optional(), // Specific quality for Sabaq Para
  quarters_revised: z.enum(["1st_quarter", "2_quarters", "3_quarters", "4_quarters"]).optional(), // Expected by sabaq_para table

  // Dhor (Revision) - from juz_revisions table (single set of fields)
  dhor_juz: z.number().min(1).max(30).optional(),
  dhor_memorization_quality: z.enum(["excellent", "good", "average", "needsWork", "horrible"]).optional(),
