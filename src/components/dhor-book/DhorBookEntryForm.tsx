...
export function DhorBookEntryForm({ onSubmit, isPending, onCancel }: DhorBookEntryFormProps) {
  ...
  function handleSubmit(data: DailyActivityFormValues) {
    ...
    console.log("Final payload for onSubmit (DhorBookCombinedFormData):", JSON.stringify(finalPayload, null, 2));
    onSubmit(finalPayload);
  }

  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4 max-h-[80vh] overflow-y-auto p-1">
        <FormField
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} defaultValue={field.value} >
                    <FormControl><SelectTrigger><SelectValue placeholder="Select Quran format" /></SelectTrigger></FormControl>
                    <SelectContent>
          ...
          <TabsContent value="sabaq-para" className="space-y-4 pt-4">
            <FormField control={form.control} name="sabaq_para_juz" render={({ field }) => (
                <FormItem>
                  <FormLabel>Sabaq Para - Juz</FormLabel>
                  <Select onValueChange={(value) => field.onChange(parseInt(value))} value={field.value?.toString()}>
                    <FormControl><SelectTrigger><SelectValue placeholder="Select Juz for Sabaq Para" /></SelectTrigger></FormControl>
                    <SelectContent>
                      {juzData?.map((juz) => (<SelectItem key={`sabaq-para-juz-${juz.id}`} value={juz.juz_number.toString()}>Juz {juz.juz_number}</SelectItem>)) 
                       || (<SelectItem disabled value="loading-juz-sabaq-para">{juzLoading ? "Loading Juz data..." : "No Juz data"}</SelectItem>)}
                    </SelectContent>
                  </Select><FormMessage />
                </FormItem>)} />
            <FormField control={form.control} name="sabaq_para_pages" render={({ field }) => (
                <FormItem>
                  <FormLabel>Sabaq Para - Pages Read</FormLabel>
                  <FormControl><Input type="number" placeholder="Enter number of pages" {...field} onChange={e => field.onChange(parseInt(e.target.value) || 0)} /></FormControl>
                  <FormMessage />
                </FormItem>)} />
            <FormField control={form.control} name="quarters_revised" render={({ field }) => (
                <FormItem>
                  <FormLabel>Sabaq Para - Quarters Revised</FormLabel>
                  <Select onValueChange={field.onChange} value={field.value} defaultValue="1st_quarter">
                    <FormControl><SelectTrigger><SelectValue placeholder="Select quarters revised" /></SelectTrigger></FormControl>
                    <SelectContent>
                      ...
                      <SelectItem value="4_quarters">4 Quarters (Full Juz)</SelectItem>
                    </SelectContent>
                  </Select>
                  <FormMessage />
                </FormItem>)} />
            <FormField control={form.control} name="sabaq_para_memorization_quality" render={({ field }) => (
                <FormItem>
          ...
          <TabsContent value="revision" className="space-y-4 pt-4">
            <FormField control={form.control} name="dhor_juz" render={({ field }) => (
                <FormItem>
                  <FormLabel>Dhor - Juz Revised</FormLabel>
                  <Select onValueChange={(value) => field.onChange(value ? parseInt(value) : undefined)} value={field.value?.toString()}>
                    <FormControl><SelectTrigger><SelectValue placeholder="Select Juz for Dhor" /></SelectTrigger></FormControl>
                    <SelectContent>
                      {juzData?.map((juz) => (<SelectItem key={`dhor-juz-${juz.id}`} value={juz.juz_number.toString()}>Juz {juz.juz_number}</SelectItem>)) 
                       || (<SelectItem disabled value="loading-juz-dhor">{juzLoading ? "Loading Juz data..." : "No Juz data"}</SelectItem>)}
                    </SelectContent>
                  </Select><FormMessage />
                </FormItem>)} />
            <div className="grid grid-cols-2 gap-4">
              <FormField control={form.control} name="dhor_quarter_start" render={({ field }) => (
                  <FormItem>
                    <FormLabel>Dhor - Starting Quarter</FormLabel>
                    <Select onValueChange={(value) => field.onChange(value ? parseInt(value) : undefined)} value={field.value?.toString()}>
                      <FormControl><SelectTrigger><SelectValue placeholder="Select starting quarter" /></SelectTrigger></FormControl>
                      <SelectContent>
              ...
              <FormField control={form.control} name="dhor_quarters_covered" render={({ field }) => (
                  <FormItem>
                    <FormLabel>Dhor - Quarters Covered</FormLabel>
                    <Select onValueChange={(value) => field.onChange(value ? parseInt(value) : undefined)} value={field.value?.toString()}>
                      <FormControl><SelectTrigger><SelectValue placeholder="Select quarters covered" /></SelectTrigger></FormControl>
                      <SelectContent>
          ...
          <TabsContent value="general" className="space-y-4 pt-4">
            ...
            <FormField control={form.control} name="comments" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Textarea placeholder="Enter any general comments" className="resize-none" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>)} />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <FormField control={form.control} name="detention" render={({ field }) => (
        ...
        {calculatedPages > 0 && (<div className="text-sm text-muted-foreground">Pages to memorize: {calculatedPages} pages</div>)}

        <DialogFooter>
          <Button type="button" variant="secondary" onClick={onCancel}>Cancel</Button>
          <Button type="submit" disabled={isPending}>{isPending ? "Creating..." : "Create Entry"}</Button>
        </DialogFooter>
      </form>
    </Form>
  );
