...
export function MasteryLevelGrid(
  { studentId, onJuzSelect }: MasteryLevelGridProps,
) {
  const { data: masteryData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("juz_revisions")
        .select(
          "id, student_id, juz_revised, memorization_quality, revision_date",
        )
      ...
      juzRevisions.forEach((revision) => {
        ...
        if (!juzMasteryMap.has(juzNumber)) {
          ...
          if (
            revision.revision_date && (!existing.last_revision_date ||
              new Date(revision.revision_date) >
                new Date(existing.last_revision_date))
          ) {
