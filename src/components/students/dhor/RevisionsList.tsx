...
import { JuzRevision, RevisionsListProps } from "@/types/progress";
import { CalendarDays, Plus } from "lucide-react";

export function RevisionsList(
  { revisions = [], studentName = "", onAddRevision }: RevisionsListProps,
) {
  if (!revisions || revisions.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-muted-foreground mb-4">
          No revision history found for this student.
        </p>
        {onAddRevision && (
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="font-medium text-lg">
          Revision History for {studentName}
        </h3>
        {onAddRevision && (
                ...
                <span
                  className={`inline-block px-2 py-1 rounded text-xs font-medium ${
                    revision.memorization_quality === "excellent"
                      ...
                      : revision.memorization_quality === "average"
                      ? "bg-yellow-100 text-yellow-800"
                      : revision.memorization_quality === "needsWork"
                      ? "bg-orange-100 text-orange-800"
                      : "bg-red-100 text-red-800"
                  }`}
                >
                  {revision.memorization_quality &&
                    revision.memorization_quality.charAt(0).toUpperCase() +
                      revision.memorization_quality.slice(1)}
                </span>
              </div>
            </div>
