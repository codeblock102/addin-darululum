...
export const StudentPerformanceMetrics = (
  ...
  >({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("attendance").select("*").eq(
        "student_id",
        studentId,
      );
  ...
  >({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("progress").select("*").eq(
        "student_id",
        studentId,
  ...
  const { data: sabaqParaEntries, isLoading: sabaqParaLoading } = useQuery<
    Tables<"sabaq_para">[]
  >({
    queryKey: ["sabaq-para-metrics", studentId],
    queryFn: async () => {
      const { data, error } = await supabase.from("sabaq_para").select("*").eq(
        "student_id",
        studentId,
  ...
  const { data: juzRevisionsEntries, isLoading: juzRevisionsLoading } =
    useQuery<Tables<"juz_revisions">[]>({
      queryKey: ["juz-revisions-metrics", studentId],
      queryFn: async () => {
        const { data, error } = await supabase.from("juz_revisions").select("*")
          .eq("student_id", studentId).order("revision_date", {
            ascending: false,
          });
        if (error) throw error;
        return data || [];
  ...
  >({
    ...
    queryFn: async () => {
      ...
      if (error) {
        console.error(
          "Error fetching student data for metrics:",
          error.message,
        );
        return null;
      }
  ...
  const isLoading = attendanceLoading || progressLoading || sabaqParaLoading ||
    juzRevisionsLoading || studentDataLoading;
