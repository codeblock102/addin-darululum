...
export function getAyahRangeForSurahInJuz(
  juzNumber: number,
  surahNumber: number,
): { startAyah: number; endAyah: number } | null {
  ...
  const juzSurahRanges: Record<
    number,
    Record<number, { startAyah: number; endAyah: number }>
  > = {
    1: {
  ...
  console.warn(
    `No ayah range mapping found for Juz ${juzNumber}, Surah ${surahNumber}. Using fallback.`,
  );
